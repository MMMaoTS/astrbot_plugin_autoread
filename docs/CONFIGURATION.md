...
| `enabled_umos` | list | **`[]`** | 拥有 AutoRead **管理/控制权限**的 UMO 列表。**默认空列表 = 不授权任何 UMO**。授权 UMO 可使用 `/read` 命令、导入/删除书籍、推进/调整阅读进度、触发平台文件自动入库等管理操作。**角色书架、阅读状态和笔记属于角色自身能力状态**，所有会话均可通过自然语言了解书架、查询阅读状态与笔记——不受此列表限制。多个 UMO 以逗号或换行分隔。 |
| `auto_import_uploaded_books` | bool | **false** | 上传文件自动入库开关。开启后，在**授权 UMO**（`enabled_umos` 命中）的会话中上传 txt/md 文件时自动导入到角色书架。**默认关闭**。建议优先通过 WebUI / 后台上传书籍，避免会话中随手上传的文件误入库导致 books/chunks/state 管理紊乱。只有明确需要平台会话中发送书籍文件即可自动入库时才开启。导入后默认不主动回复。 |
