...
- **WebUI 管理页面**：书籍管理、笔记查看与删除、阅读任务管理、设置同步、备份导入/导出/管理
- **WebUI 删除功能**（默认关闭，需在设置中开启 `webui_delete_enabled`，前后端双重校验）
- **服务器备份管理**：查看、下载、上传、恢复、删除服务器备份文件
- **最后错误管理**：自动过期（TTL 可配置）+ 手动清除
- 自然语言不能删除笔记（管理操作仅限 WebUI 和命令入口）
...
- 删除是永久操作，但可通过备份恢复找回（前提是之前导出过包含该笔记的备份）。
